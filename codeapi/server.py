...
MAILDIR = Path(os.environ.get(
    "CODEAPI_MAILDIR",
    "/var/mail/vhosts/veximail.xyz/catchall/Maildir",
))
API_TOKEN = os.environ.get("CODEAPI_TOKEN", "")
DOMAIN = os.environ.get("CODEAPI_DOMAIN", "veximail.xyz")

if not API_TOKEN:
    raise SystemExit("CODEAPI_TOKEN env var required")

# Алфавит локальной части генерируемого адреса: строчные буквы + цифры.
# Без точек/спецсимволов — чтобы адрес гарантированно прошёл везде.
_ADDR_ALPHABET = string.ascii_lowercase + string.digits


def new_address(length=10):
    """Сгенерировать случайный адрес *@DOMAIN.

    Сервер — catch-all, поэтому ящик заранее создавать не нужно: любое
    письмо на этот адрес упадёт в общий Maildir, а фильтр /codes?to=...
    выберет письма именно для него.
    """
    local = "".join(secrets.choice(_ADDR_ALPHABET) for _ in range(length))
...
def address_new():
    require_auth()
    try:
        length = max(4, min(32, int(request.args.get("length", "10"))))
    except ValueError:
        length = 10
    return jsonify({"address": new_address(length), "domain": DOMAIN})
