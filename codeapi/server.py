...
_KEYWORDS = (
    ...
    r"пин|pin|otp|otc|одноразов|verifizier"
)
...
def collect(service=None, limit=50):
    items = []
...
def list_codes():
    ...
    try:
        ...
        limit = 20
    return jsonify(collect(service=service, limit=limit))
...
def latest_code():
    require_auth()
    service = request.args.get("service")
    items = collect(service=service, limit=1)
    if not items:
        abort(404)
